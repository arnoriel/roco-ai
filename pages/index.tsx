...
export default function Home() {
  ...
  const handleSend = async () => {
    ...
    if (!sessionId) {
      ...
      const newSession = { id: newId, title: input.substring(0, 30), messages: [], createdAt: Date.now() };
      setSessions([newSession]);
      setActiveSessionId(newId);
      sessionId = newId;
    }
    const userMsg = { role: "user", content: input };
    setSessions(prev => prev.map(s => s.id === sessionId ? { ...s, messages: [...s.messages, userMsg], title: s.messages.length === 0 ? input.substring(0, 30) : s.title } : s));
    setInput("");
    setIsLoading(true);
    try {
  ...
  return (
    <div className={`min-h-screen flex font-sans transition-colors duration-300 ${isDarkMode ? "bg-[#171717] text-white" : "bg-white text-slate-900"}`}>
      
      {/* SIDEBAR */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 transform transition-transform duration-300 ease-in-out border-r shadow-2xl
        ${isDarkMode ? "bg-[#171717] border-white/10" : "bg-slate-50 border-slate-200"}
        ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        
        <div className="flex flex-col h-full p-4">
          <button onClick={() => { const newId = Date.now().toString(); setSessions([{ id: newId, title: "Chat Baru", messages: [], createdAt: Date.now() }, ...sessions]); setActiveSessionId(newId); setIsSidebarOpen(false); }} className={`w-full mb-6 p-3 rounded-xl border flex items-center justify-center gap-2 font-medium transition-all
            ...
            {sessions.map((s) => (
              <div key={s.id} className="relative group">
                ...
                <button onClick={(e) => { e.stopPropagation(); setActiveMenuId(activeMenuId === s.id ? null : s.id); }}
                  className={`absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-md hover:bg-black/10 dark:hover:bg-white/10 transition-opacity ${activeSessionId === s.id ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
          ...
          <div className={`mt-4 pt-4 border-t ${isDarkMode ? "border-white/10" : "border-slate-200"}`}>
            <div className="flex flex-col gap-1 px-2 mb-4">
              <span className="text-[11px] font-bold text-slate-400 tracking-tight">Roco AI v.1.1</span>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-sm rounded-2xl shadow-2xl border p-6 animate-in fade-in zoom-in duration-200 ${isDarkMode ? "bg-[#252525] border-white/10" : "bg-white border-slate-200"}`}>
            <h3 className="text-lg font-bold mb-2">
              {modalConfig.type === "rename" ? "Rename Chat" : modalConfig.type === "delete" ? "Delete Chat" : "Clear All History"}
            </h3>
            <p className="text-sm text-slate-500 mb-6">
              {modalConfig.type === "rename" ? "Masukkan nama baru untuk percakapan ini." : "Tindakan ini tidak dapat dibatalkan. Apakah Anda yakin?"}
            </p>
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={closeModal} className={`px-4 py-2 rounded-lg text-sm font-medium ${isDarkMode ? "hover:bg-white/5" : "hover:bg-slate-100"}`}>Batal</button>
              <button 
                onClick={modalConfig.type === "rename" ? handleRenameAction : modalConfig.type === "delete" ? handleDeleteAction : handleClearAllAction}
                className={`px-4 py-2 rounded-lg text-sm font-bold text-white transition-all ${modalConfig.type === "rename" ? "bg-blue-600 hover:bg-blue-700" : "bg-red-600 hover:bg-red-700"}`}>
                Konfirmasi
              </button>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <nav className={`border-b py-3 px-6 flex justify-between items-center z-20 backdrop-blur-md ${isDarkMode ? "border-white/10 bg-[#171717]/80" : "border-slate-200 bg-white/80"}`}>
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(true)} className="group flex items-center gap-2 outline-none">
              <span className="font-bold text-lg tracking-tight group-hover:text-blue-500 transition-colors">Roco AI</span>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-4 h-4 text-slate-400 group-hover:text-blue-500 transition-all ${isSidebarOpen ? 'rotate-180' : ''}`}><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
            </button>
          </div>
          ...
          <div className="max-w-3xl mx-auto p-4 space-y-8 pb-32">
            {chatLog.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center pt-24 text-slate-400 space-y-3">
                ...
                <p className="text-sm font-medium animate-in fade-in slide-in-from-bottom-2 duration-700">{randomOpening}</p>
              </div>
            )}
            {chatLog.map((chat, i) => (
              <div key={i} className={`flex gap-4 ${chat.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[14.5px] leading-relaxed ${chat.role === "user" ? (isDarkMode ? "bg-[#2f2f2f]" : "bg-slate-100") : ""}`}>
                  <div className={`prose max-w-full overflow-hidden break-words ${isDarkMode ? "prose-invert" : "prose-slate"}`}>
                    <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                      pre({ children }: any) {
                        const codeString = children?.props?.children || "";
                        return (
                          <div className={`relative my-4 rounded-xl border overflow-hidden ${isDarkMode ? "border-white/10" : "border-slate-200"}`}>
                            <div className={`px-4 py-1.5 border-b flex justify-between items-center text-[10px] ${isDarkMode ? "bg-[#252525] border-white/10" : "bg-slate-50 border-slate-200"}`}>
                              <span className="font-mono opacity-50 uppercase tracking-tighter">code snippet</span>
                              <button onClick={() => copyCode(String(codeString).replace(/\n$/, ""))} className="hover:text-blue-500 font-bold transition-colors">Copy</button>
                            </div>
                            <pre className="bg-[#0D0D0D] p-4 overflow-x-auto text-[13px] text-slate-300 custom-scrollbar">{children}</pre>
                          </div>
                        );
                      }
            ...
            {isLoading && <div className="flex gap-2 items-center text-[11px] text-slate-500 animate-pulse ml-12"><div className="w-1 h-1 bg-slate-500 rounded-full animate-bounce" /><span>Roco AI sedang mengetik...</span></div>}
            <div ref={chatEndRef} />
          </div>
        </main>

        <div className={`p-4 border-t ${isDarkMode ? "bg-[#171717] border-white/5" : "bg-white border-slate-100"}`}>
          <div className="max-w-3xl mx-auto flex flex-col gap-2">
            <div className="relative flex items-center">
              <input className={`w-full p-4 pr-16 rounded-2xl shadow-sm focus:outline-none transition-all text-[15px] ${isDarkMode ? "bg-[#2f2f2f] border-white/10 text-white placeholder:text-slate-500" : "bg-white border-slate-300 text-slate-900 shadow-sm placeholder:text-slate-400"}`} placeholder="Tanya Roco AI..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} />
              <button onClick={handleSend} disabled={isLoading} className={`absolute right-3 p-2 rounded-xl transition-all ${isDarkMode ? "bg-white text-black hover:bg-slate-200" : "bg-slate-900 text-white hover:bg-slate-800"} disabled:opacity-20`}>
            ...
            <p className="text-[10px] text-center text-slate-500 font-medium tracking-tight">Roco AI can make mistakes, so double check-it.</p>
          </div>
        </div>
      </div>

      {/* Overlay Sidebar */}
      {isSidebarOpen && <div onClick={() => setIsSidebarOpen(false)} className="fixed inset-0 bg-black/40 backdrop-blur-sm z-30" />}
