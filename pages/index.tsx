...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen flex font-sans transition-colors duration-300 ${isDarkMode ? "bg-[#171717] text-white" : "bg-white text-slate-900"}`}>
      ...
      {/* SIDEBAR */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 transform transition-transform duration-300 ease-in-out border-r shadow-2xl
        ${isDarkMode ? "bg-[#171717] border-white/10" : "bg-slate-50 border-slate-200"}
        ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        
        <div className="flex flex-col h-full p-4">
          <button onClick={() => { const newId = Date.now().toString(); setSessions([{ id: newId, title: "Chat Baru", messages: [], createdAt: Date.now() }, ...sessions]); setActiveSessionId(newId); setIsSidebarOpen(false); }} 
          ...
          <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar pr-2">
            ...
            {sessions.map((s) => (
              <div key={s.id} className="relative group">
                <button onClick={() => { setActiveSessionId(s.id); setIsSidebarOpen(false); }}
      ...
      {modalConfig.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-sm rounded-2xl shadow-2xl border p-6 animate-in fade-in zoom-in duration-200 ${isDarkMode ? "bg-[#252525] border-white/10" : "bg-white border-slate-200"}`}>
            <h3 className="text-lg font-bold mb-2">{modalConfig.type === "rename" ? "Rename Chat" : "Delete Chat"}</h3>
            <p className="text-sm text-slate-500 mb-6">{modalConfig.type === "rename" ? "Ganti nama sesi riset ini." : "Yakin ingin menghapus sesi ini?"}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="max-w-3xl mx-auto p-4 space-y-8 pb-32">
              ...
              <div key={i} className={`flex gap-3 md:gap-4 group ${chat.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div className="flex flex-col gap-1.5 max-w-[88%] md:max-w-[85%]">
                  ...
                  <div className={`flex items-center gap-4 px-1 ${chat.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                    <button onClick={() => copyToClipboard(chat.content, i)} className="flex items-center gap-1.5 text-slate-400 hover:text-blue-500 transition-colors p-1">
                      {copiedIndex === i ? (
                        <span className="text-[10px] font-bold animate-in fade-in text-blue-500">Copied!</span>
                      ) : (
                    ...
                    {chat.role === "user" && (
                      <button onClick={() => handleEdit(i, chat.content)} className="text-slate-400 hover:text-blue-500 transition-colors p-1">
            ...
            {isLoading && (
              <div className="flex gap-2 items-center text-[11px] text-slate-500 animate-pulse ml-11">
                <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce" />
                <span>Roco sedang mengetik...</span>
              </div>
            )}
            <div ref={chatEndRef} />
          </div>
              ...
              <textarea ref={textareaRef} rows={1} className={`w-full p-4 pr-14 rounded-2xl shadow-sm focus:outline-none transition-all text-[15px] resize-none overflow-y-auto max-h-40 ${isDarkMode ? "bg-[#2f2f2f] border-white/10 text-white" : "bg-white border-slate-300 text-slate-900 shadow-sm"}`} placeholder={editingIndex !== null ? "Edit pesanmu..." : "Tanya Roco AI..."} value={input}
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !e.shiftKey) {
                    e.preventDefault();
                    if (editingIndex !== null) handleSend(input, true, editingIndex);
                    else handleSend();
                  }
                }}
              />
              <button onClick={() => isLoading ? null : (editingIndex !== null ? handleSend(input, true, editingIndex) : handleSend())} className={`absolute right-3 bottom-3 p-2 rounded-xl transition-all ${isDarkMode ? "bg-white text-black" : "bg-slate-900 text-white"}`}>
                {isLoading ? (
      ...
      {isSidebarOpen && <div onClick={() => setIsSidebarOpen(false)} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 transition-opacity" />}
